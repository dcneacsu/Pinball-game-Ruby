...
def update(racquet,screen,testmode)
    ...
    if ((racquet+2<=$x.floor || racquet-1>=$x.floor) && $y.floor==15-1)
        return nil
...
$star=Array.new(SCREEN_X){ Array.new(SCREEN_Y, 0)}
$counter=0
...
def displayDyn(screen,racquet)
      ...
      if (screen[$y.floor][$x.floor]==SC_BLANK)
        $counter+=1
        $star[$y.floor][$x.floor]+=1
    end
    if (($star[$y.floor][$x.floor]) > 2 )&& (($star[$y.floor][$x.floor])> ($counter/50))
        screen[$y.floor][$x.floor]=SC_STAR
        print "\e[#{1+$y.floor};#{$x.floor}H"
    end
  ...
  if $y >= 0 && $x >= 0 && $y < SCREEN_Y && $x < SCREEN_X
    ...
    (2...SCREEN_X).each do|m|
        print "\e[#{SCREEN_Y};#{m}H-"
    (0...4).each do |n|
        print "\e[#{SCREEN_Y};#{racquet+n-1}H="
    end
